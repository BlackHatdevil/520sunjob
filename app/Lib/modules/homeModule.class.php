...
class homeModule extends BaseModule
{
    ...
    public function user_resume(){
        ...
        /*此处有个小漏洞，是否需要解决只能查看自己报名者的简历*/
        $id = $_REQUEST['id'];
        $user=$GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user where is_effect=1 and id=".$id);
        if(!$user){
            showErr("错误：找不到该用户");
        }
        $resume = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user_resume where user_id=".$id);
        if(!$resume)
            showErr("错误：找不到该用户简历");
        $GLOBALS['tmpl']->assign('resume',$resume);		
        $GLOBALS['tmpl']->display("user_resume.html");
    }
