...
class userModule extends BaseModule
{
	...
	{		
        ...
        //检测是否为企业和有效性
        $is_company = $GLOBALS['db']->getOne("select is_company from ".DB_PREFIX."user where (email ='".$_REQUEST['email']."' or user_name='".$_REQUEST['email']."')");
        $is_effect = $GLOBALS['db']->getOne("select is_effect from ".DB_PREFIX."user where (email ='".$_REQUEST['email']."' or user_name='".$_REQUEST['email']."')");
        if(!$is_company) showErr("非企业级用户，请在手机客户端登录",$ajax,"");
        if(!$is_effect) showErr("企业级别用户：请耐心等待审核",$ajax,"");
	...
	{		
		...
		$user_data['is_effect'] = 0;//企业需要审核
		$user_data['is_company'] = 1;
		...
		if($res['status'] == 1)
		{
			...
			{
				ajax_return(array("status"=>0,"info"=>"<font color='#f90'>企业用户，请等待管理员审核！</font>"));
			}
		}
		else
		{
	...
	public function do_api_register()
	{
		...
		if($res['status'] == 1)
		{
			...
			$user_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where id = ".$user_id);
			if($user_info['is_effect']==1&&$user_info['is_company']==1)
			{
				//在此自动登录
				do_login_user($user_data['email'],$user_data['user_pwd']);
				ajax_return(array("status"=>1,"jump"=>get_gopreview()));
			...
			{
				ajax_return(array("status"=>0,"info"=>"企业用户：请等待管理员审核","jump"=>get_gopreview()));
			}
		}
		else
		{
